...
// запроса сервисом. Ошибка заносится в лог, отправителю возвращается ответ с кодом http.StatusUnauthorized. При
// валидном token-е производится проверка на существование в теле токена разрешения на CRUD операцию, соответствующую
// http-методу запроса. Разрешения "c", "r", "u", "d" должны иметь значение true, чтобы считаться выданными. При
// несоответствии или отсутствии разрешения, прекращается обработка запроса, ошибка выводится в лог, а отправителю
// возвращается ошибка http.StatusForbidden.
...
// При невозможности извлечь из тела токена разрешения или при несоответствии разрешений переданному методу,
// возвращается ошибка.
